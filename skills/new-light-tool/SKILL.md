...
- **`versionName`** must be strict `x.y.z` — no pre-release suffix, no build metadata, no leading zeros. Two different gates enforce this at different strictness, which is worth knowing: `reusable-submission-check.yml` uses `^[0-9]+\.[0-9]+\.[0-9]+$` and so fails the PR on a suffix or a missing component, but it *accepts* leading zeros. The SDK's own `LightToolPolicy.VERSION_NAME_PATTERN` rejects leading zeros and runs at Gradle-configure time, so `01.2.3` clears the PR and then fails the build. Avoid leading zeros and neither gate is in play. Start at `0.1.0`; `1.0.0` only if the first release is genuinely shipping-ready.
...
Consequence to internalize immediately: **that `upstream` remote is why every `gh` call needs its repo stated explicitly.** With no default set, `gh` can resolve the base repo to `lightphone/light-sdk` and point writes at a repo that must stay read-only. A misdirected write usually surfaces as a baffling permissions error; a misdirected `gh pr create` surfaces instead as `No commits between main and <branch>` plus `Head sha can't be blank`, which reads like a bad branch but is the same root cause. Pass `-R tyleryancey/light-<tool>` (or the repo as a positional arg to `gh repo view`, or in the path for `gh api`) on every single call.
